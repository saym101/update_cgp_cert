...
check_dependencies() {
    ...
    if [ ${#missing[@]} -ne 0 ]; then
        ...
        echo "Установите их командой: apt update && apt install ${missing[*]} -y"
        exit 1
    fi
}
...
task_get_certs() {
    ...
    for domain in $DOMAINS_BASE; do
        ...
        if certbot certonly --standalone -d "mail.$domain" --key-type rsa --rsa-key-size 2048 --non-interactive --agree-tos --email "$EMAIL_ADMIN" > "$cb_out" 2>&1; then
            ...
            log_msg "[FAIL] mail.$domain: ОШИБКА Certbot!"
        fi
        rm -f "$cb_out"
    done
...
task_install_to_cgp() {
    ...
    for domain in $DOMAINS_BASE; do
        ...
        if resp=$(curl -s -u "$EMAIL_ADMIN:$SMTP_PASS" -k "http://$IP_CGP:$CLI_PORT/cli/" --data-urlencode "$CMD") && [[ ! "$resp" =~ "ERROR" ]]; then
            log_msg "[OK] $domain: Синхронизация успешна (из $path)."
        else 
            log_msg "[ERROR] $domain: Сервер отклонил данные. Ответ: $resp"
        fi
    done
}
...
task_cleanup_manual() {
    ...
    echo "2) АВТО-ОЧИСТКА всех битых конфигов"
    ...
    case "$clean_ch" in
        1)
            ...
            read -r -p "Введите полное имя для удаления: " cert_to_del
